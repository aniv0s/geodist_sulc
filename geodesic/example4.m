function[distances, paths] = example4(surf, source)
...
for i=1:N   
    ...
    if(d < 1e10)        %if distance is found, we can trace a path
        paths{i} = geodesic_trace_back(algorithm, q);     %find a shortest path from source to destination
    end
    disp(i);
end;
% 
