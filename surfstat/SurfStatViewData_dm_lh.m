function [ a, cb ] = SurfStatViewData_dm_lh( data, surf, title, background );
...
lighting phong; material dull; shading interp; % interp; % material metal    

a(2)=axes('position',[-0.05 0 h w]);%1-0.055-h*3/4 0.62 h*3/4 w]);
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
    double(data(vl)),'EdgeColor','none');
view(90,0);
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
lighting phong; material dull; shading interp ;% material metal faceted     
