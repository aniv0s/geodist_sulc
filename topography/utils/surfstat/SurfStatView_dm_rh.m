function [ a, cb ] = SurfStatView_dm_rh( struct, surf, title, background)
...
if ~isstruct(struct)
    [a,cb]=SurfStatViewData_dm_rh(struct,surf,title,background);
else
    if isfield(struct,'P')
        ...
        [a,cb]=SurfStatViewData_dm(tt,surf,title,background);
        cm=[zeros(1,3);
...
structNoZeros = struct(struct ~= 0);
sorted = sort(structNoZeros);
maxVal = (96/100)*length(sorted)+0.5;
minVal = (4/100)*length(sorted)+0.5;
max = sorted(floor(maxVal));
min = sorted(floor(minVal));
disp(min);
disp(max);
%    SurfStatColLim([0 max(sorted)]);
   SurfStatColLim([min max]);
   load('/scr/litauen1/scripts/MyColormaps','mycmap');
   SurfStatColormap(mycmap);
% [0 0 0; rand([300 3])]
%   SurfStatColormap(mycmap);
   %colormapeditor ;
return
