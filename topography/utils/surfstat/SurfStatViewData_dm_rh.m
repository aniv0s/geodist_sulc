function [ a, cb ] = SurfStatViewData_dm_rh( data, surf, title, background );
...
a(1)=axes('position',[0.45 0.0 h w]);
...
lighting phong; material shiny; shading interp; %interp;

a(2)=axes('position',[-0.05 0.0 h w]);%1-0.055-h*3/4 0.29 h*3/4 w]);
trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
    double(data(vr)),'EdgeColor','none');
view(90,0);
daspect([1 1 1]); axis tight;  axis vis3d off;
...
% a(3)=axes('position',[0.45 0.45 h w]);
% trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
%     double(data(vl)),'EdgeColor','none');
% view(-90,0); 
% daspect([1 1 1]); axis tight; axis vis3d off;
% lighting phong; material shiny; shading interp; % interp; % material metal    
% 
% a(4)=axes('position',[-0.05 0.45 h w]);%1-0.055-h*3/4 0.62 h*3/4 w]);
% trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
%     double(data(vl)),'EdgeColor','none');
% view(90,0);
% daspect([1 1 1]); axis tight; axis vis3d off;
% lighting phong; material shiny; shading interp ;% material metal faceted     
% 



% 
% a(2)=axes('position',[0.3 0.7 h w]);
...
set(cb,'Position',[0.35 0.055 0.3 0.01]);
